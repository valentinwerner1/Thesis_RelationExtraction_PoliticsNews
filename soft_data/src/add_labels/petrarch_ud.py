...
def main():
    ...
    if cli_args.command_name == 'parse' or cli_args.command_name == 'batch':
        ...
        if cli_args.config:
            ...
            PETRreader.parse_Config(utilities._get_data('config/','PETR_config.ini'))
        ...
        if cli_args.config:
            ...
            logger.info('Using default config file.')
            PETRglobals.ConfigFileName = 'PETR_config.ini'
            PETRreader.parse_Config(utilities._get_data('config/',
                                                        'PETR_config.ini'))
...
def read_dictionaries(validation=False):
    ...
    pico_path = utilities._get_data('dictionaries', PETRglobals.InternalCodingOntologyFileName)
    ...
    for actdict in PETRglobals.ActorFileList:
        actor_path = utilities._get_data('dictionaries', actdict)
    ...
    for agentdict in PETRglobals.AgentFileList:
        agent_path = utilities._get_data('dictionaries', agentdict)
        PETRreader.read_agent_dictionary(agent_path)

    print('Discard dictionary:', PETRglobals.DiscardFileName)
    discard_path = utilities._get_data('dictionaries',
    ...
    if PETRglobals.IssueFileName != "":
        ...
        issue_path = utilities._get_data('dictionaries',
...
def gen_cameo_event(jsonString):
    ...
    if events:
        petr_ud_results = do_coding(events)
        return petr_ud_results
    else:
        return {}

def gen_cameo_event_dict(corenlp_dict):
    events = PETRreader.read_dict(corenlp_dict)
    if events:
        petr_ud_results = do_coding(events)
        return petr_ud_results
    else:
