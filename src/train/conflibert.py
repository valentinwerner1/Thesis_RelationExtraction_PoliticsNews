...
class GetData(pl.LightningDataModule):
    ...
    def train_dataloader(self, *args, **kwargs): 
        ...
        return DataLoader(self.train_dataset, batch_size = self.conf.batch_size, collate_fn = self.data_collator, shuffle = True, num_workers= 0)
    ...
    def val_dataloader(self, *args, **kwargs): 
        ...
        return DataLoader(self.val_dataset, batch_size = self.conf.batch_size, collate_fn = self.data_collator, num_workers= 0)
    ...
    def test_dataloader(self, *args, **kwargs): 
        ...
        return DataLoader(self.test_dataset, batch_size = self.conf.batch_size, collate_fn = self.data_collator, num_workers= 0)
...
class GenerateTextSamplesCallback(Callback):
    ...
    def on_train_batch_end(self,trainer: Trainer,pl_module: LightningModule, outputs: Sequence, batch: Sequence, batch_idx: int) -> None:
        ...
        #log predictions into wandb table
        #creates a lot of data that is not necessarily needed; can be commented out
...
def train(conf=None):
    ...
    pl.seed_everything(0)   #this was mistakenly kept at 0 all time; noticed too late with too little time to fix
    ...
    #tokenizer and model loaded from Huggingface
    ...
    #initialise ConfliBERT as encoder-decoder transformer
    ...
    if sys.argv[2] == "finetune":
        if sys.argv[1] == "pentacode":
            ckpt = torch.load("models/ConfliBERT_pretrain_Pentacode/epoch=29-step=14970.ckpt", map_location={'cuda:0':'cuda:2'})
        elif sys.argv[1] == "cameo":
            if sys.argv[4] == "gold" or sys.argv[4] == "spacy":
                ckpt = torch.load("models/ConfliBERT_pre-train_ents/epoch=29-step=14970.ckpt", map_location={"cuda:1":"cuda:2"})
            else:
                ckpt = torch.load("models/ConfliBERT_pretrain_CAMEO.pth/epoch=28-step=14471.ckpt", map_location={'cuda:0':'cuda:2'})
        else: 
    ...
    wandb_logger = WandbLogger(project = "project/ConfliBERT".split('/')[-1].replace('.py', ''), name = f"{sys.argv[1]},{sys.argv[2]},seed{sys.argv[3]};{sys.argv[6]}", log_model = False)
