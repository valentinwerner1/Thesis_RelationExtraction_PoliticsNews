...
parameters_dict = {
    ...
    'lr': {'values': [0.000025, 0.000035, 0.00005, 0.000065, 0.000075]},
    "lr_decay" : {"values":[0.05, 0.1, 0.15, 0.2]},
    "eps_loss" : {"values": [0, 0.05, 0.1, 0.15, 0.2]},
    "weight_decay" : {"values" : [0.0025, 0.005, 0.0075, 0.01, 0.015]},
    "masking_rate" : {"values" : [0, 0.1, 0.2]}
    }
    
sweep_config['parameters'] = parameters_dict

sweep_id = wandb.sweep(sweep_config, project="ConfliBERT_hparams_finetune")
...
class GetData(pl.LightningDataModule):
    ...
    def preprocess_function(self, data):
        ...
        #process input
        model_inputs = self.tokenizer(inputs, max_length = 192, padding = True, truncation = True)

        #process labels
        with self.tokenizer.as_target_tokenizer():
            labels = self.tokenizer(outputs, max_length = 192, padding = True, truncation = True)
...
class BaseModule(pl.LightningModule):
    ...
    def training_step(self, batch: dict, batch_idx: int):
        ...
        if labels.shape[-1] < 192:
            forward_output['labels'] = self._pad_tensors_to_max_len(labels, 192)
        else:
    ...
    def generate_triples(self, batch, labels) -> None:
        ...
        generated_tokens = self.model.generate(
            ...
            use_cache = True, max_length = 192, early_stopping = False, length_penalty = 1, 
...
def re_score(pred_relations, gt_relations):
    ...
    "exhibit military posture","reject","protest","reduce relations","coerce","assault","fight"]#"engage in unconvential mass violence"]
...
def train(conf=None):
    ...
    with wandb.init(config=conf):
        ...
        ckpt = torch.load("models/ConfliBERT_pretrain_CAMEO.pth/epoch=28-step=14471.ckpt", map_location={'cuda:0':'cuda:0'})
        ...
        wandb_logger = WandbLogger(project = "project/ConfliBERT_hparams_finetune".split('/')[-1].replace('.py', ''), name = "ConfliBERT", log_model = False)
